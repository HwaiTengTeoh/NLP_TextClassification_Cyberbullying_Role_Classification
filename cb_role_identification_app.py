...
def get_cb_role_model():

    # Define pretrained tokenizer and model
    tokenizer = AutoTokenizer.from_pretrained('distilbert-base-uncased')
    model = AutoModelForSequenceClassification.from_pretrained('teoh0821/cb_role_identification', num_labels=4)
...
if selected == "Application":   
    ...
    with st.spinner("Setting up.."):
        tokenizer, model = get_cb_role_model()
    ...
    if input_text and button:
        ...
        with st.spinner("Almost there.. Analyzing your input text.."):
            ...
            numeric_label = [np.argmax(raw_pred, axis=1)==0, 
                   np.argmax(raw_pred, axis=1)==1,
                   np.argmax(raw_pred, axis=1)==2,
                   np.argmax(raw_pred, axis=1)==3]
            
            text_label = ["Non-bully", "Harasser","Victim","Bystander (defender)"]
         
            text_pred = np.select(numeric_label, text_label)


            if text_pred.tolist()[0] == "Non-bully":
